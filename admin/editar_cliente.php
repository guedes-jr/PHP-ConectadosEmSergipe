...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf()) {
        ...
        if (!$nome || !$email) {
            ...
            try {
                $stmt = $pdo->prepare("UPDATE clientes SET nome = ?, email = ?, telefone = ?, whatsapp = ?, cep = ?, rua = ?, bairro = ?, cidade = ?, estado = ?, regiao = ?, tipo = ?, cnpj = ? WHERE id = ?");
                $stmt->execute([$nome, $email, $telefone, $whatsapp, $cep, $rua, $bairro, $cidade, $estado, $regiao, $tipo, $cnpj, $id]);
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:1.5rem;">
            ...
            <div style="display:flex; flex-direction:column; gap:0.5rem;">
                ...
                <select name="tipo" required style="padding:0.75rem 1rem; border:1px solid var(--border); border-radius:0.75rem; background:var(--muted-bg); color:var(--foreground);">
                    <option value="prestador" <?php echo ($client['tipo'] ?? 'prestador') === 'prestador' ? 'selected' : ''; ?>>Prestador de Serviço</option>
                    <option value="loja" <?php echo ($client['tipo'] ?? '') === 'loja' ? 'selected' : ''; ?>>Loja / Comércio</option>
                </select>
            </div>

            <div style="display:flex; flex-direction:column; gap:0.5rem;">
                <label style="font-size:0.875rem; font-weight:600;">CNPJ (Opcional)</label>
                <input type="text" name="cnpj" value="<?php echo e($client['cnpj'] ?? ''); ?>" placeholder="00.000.000/0000-00" style="padding:0.75rem 1rem; border:1px solid var(--border); border-radius:0.75rem; background:var(--muted-bg); color:var(--foreground);">
            </div>
            ...
            <div style="display:flex; flex-direction:column; gap:0.5rem;">
                ...
                <input type="text" name="estado" id="estado" value="<?php echo e($client['estado'] ?? 'Sergipe'); ?>" placeholder="Sergipe" style="padding:0.75rem 1rem; border:1px solid var(--border); border-radius:0.75rem; background:var(--muted-bg); color:var(--foreground);">
            </div>
        </div>
